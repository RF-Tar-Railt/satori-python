...
async def group_admin_set(login: Login, net: OneBotNetwork, raw: dict):
    ...
    member = Member(
        ...
        avatar=USER_AVATAR_URL.format(uin=raw["user_id"]),
        roles=[Role("ADMINISTRATOR", "管理员")],
    )
    return Event(
...
async def member_kick_me(login: Login, net: OneBotNetwork, raw: dict):
    ...
    member = Member(
        login.user,
        member_info.get("card", member_info["nickname"]),
        USER_AVATAR_URL.format(uin=raw["user_id"]),
...
async def group_increase(login: Login, net: OneBotNetwork, raw: dict):
    ...
    if str(raw["user_id"]) == login.user.id:  # bot self joined new group
        return Event(
            ...
            user=login.user,
            member=Member(
                login.user, avatar=USER_AVATAR_URL.format(uin=login.user.id), roles=[Role("MEMBER", "群成员")]
            ),
            guild=guild,
            channel=channel,
        )
