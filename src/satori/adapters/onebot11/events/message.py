...
async def private_friend(login: Login, net: OneBotNetwork, raw: dict):
    ...
    user = User(str(sender["user_id"]), sender["nickname"], sender.get("card"), USER_AVATAR_URL.format(uin=sender["user_id"]))
...
async def private_group(login: Login, net: OneBotNetwork, raw: dict):
    ...
    return Event(
        ...
        guild=Guild(str(group_id), group_info.get("group_name"), avatar=GROUP_AVATAR_URL.format(group=group_id)) if group_id else None,
        channel=channel,
        message=MessageObject(str(raw["message_id"]), await decode(raw["message"], net)),
    )
...
async def group(login: Login, net: OneBotNetwork, raw: dict):
    ...
    member_info = await net.call_api("get_group_member_info", {"group_id": raw["group_id"], "user_id": sender["user_id"]})
    member = Member(user, member_info["card"], USER_AVATAR_URL.format(uin=sender["user_id"]))
